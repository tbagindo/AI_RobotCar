...
void AI_RobotCar::control(int X, int X0,int Y,int Y0){
  ...
  Serial.println("_th : " + String(_th));
  
  
  
  if(Y<Y0-_th){
    Serial.println("Y : " + String(Y));
    Serial.println("Y0-_th : " + String(Y0-_th));
  ...
  if(Y>=(Y0-_th) && Y<(Y0+_th)){
    _speed = 0;
    Serial.println("Speed : 2 stop");
  }
  if(Y>=Y0+_th){
    _speed = map(Y,Y0+_th,1023,0,100);
    Serial.println("Speed : 3 forward");
  }
  ...
  Serial.println("_dirL :  " + String(_dirL));
  Serial.println("_dirR :  " + String(_dirR));
  Serial.println("_speedL :  " + String(_speedL));
  Serial.println("_speedR :  " + String(_speedR));
